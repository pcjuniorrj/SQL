...
*******************************************************************************************************************************/
--	1) Fazer um Replace(CTRL + H) no caminho "F:\Temp" e alterar para um Caminho Real no servidor.
...
BEGIN
	...
	DECLARE @Id_Alerta_Parametro INT = (SELECT Id_Alerta_Parametro FROM [Monitoramento].[dbo].Alerta_Parametro (NOLOCK) WHERE Nm_Alerta = 'Trace Queries Demoradas')
	...
	*******************************************************************************************************************************/
	exec @rc = sp_trace_create @TraceID output, 0, N'F:\Temp\Duracao', @maxfilesize, NULL 
...
EndTime, Reads,Writes, CPU, ServerName, DataBaseName, RowCounts, SessionLoginName
FROM :: fn_trace_gettable(''F:\Temp\Duracao.trc'', default)
where Duration is not null
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep 
			...
			@os_run_priority = 0,
			@subsystem=N'CmdExec',
			@command = N'Del "F:\Temp\Duracao.trc" /Q',
			@flags = 0
